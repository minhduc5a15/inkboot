...
export default function PlotTrackerPage({
  ...
  const fetchEvents = useCallback(async () => {
    try {
      const res = await fetch(
        `${(process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_URL) || 'http://localhost:4000'}/novels/${novelId}/timeline`
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingEvent
      ? `${(process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_URL) || 'http://localhost:4000'}/timeline/${editingEvent.id}`
      : `${(process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_URL) || 'http://localhost:4000'}/timeline`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(
        `${(process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_URL) || 'http://localhost:4000'}/timeline/${id}`,
        { method: 'DELETE' }
      );
      if (res.ok) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await fetch(
        `${(process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_URL) || 'http://localhost:4000'}/timeline/${activeEvent.id}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
